 ...
 *  A phase may hold more than one task table (e.g. an amend round's tasks appended
 *  after a narrative note splits the section into two pipe-row runs) — a header row
 *  is identified by lookahead (the next non-table-row line is a separator row),
 *  not by a one-shot latch, so a second table's own header isn't miscounted as data. */
function parsePlan(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (sepRe.test(trimmed)) continue; // table separator row
    const next = (lines[i + 1] || '').trim();
    if (sepRe.test(next)) continue; // this row's own separator follows -- it's a header row
