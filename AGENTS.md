...
- **Each agent writes only what its contract gives it.** `contracts/<role>.yaml` carries `write`,
  ...
  where identity is certain, and `.Codex/hooks/block-path-permissions.js` reads the role from
...
Every agent also reads `_docs/status.md` when it starts and regenerates it (`node .Codex/scripts/generate-status.js` — T51, `policies/documentation.md` §2) when it finishes, rather than hand-editing it — that's left out of the table above rather than repeated on all eleven rows.
...
claiming one directory, and a hook sitting in `.Codex/hooks/` that `settings.json` never wires
up. Run it with `node orchestrator/dist/cli.js --check-layout`.

Two paths are deliberately **not** moved by it. `.Codex/agents/` is where Codex resolves
